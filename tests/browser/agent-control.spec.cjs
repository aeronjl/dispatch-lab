...
test('checkpoint recovery, continuation and recorded-request replay preserve the source',async({page})=>{
 await start(page,2);
 await page.locator('[data-ac=preview]').click();await expect(page.locator('.ac-prediction')).toContainText('Prediction only');await page.locator('[data-ac=reason]').fill('Complete the first interval');await page.locator('[data-ac=advance]').click();await expect(page.locator('[data-ac-state]')).toContainText('1 / 2 h');await expect(page.locator('[data-ac-state]')).toContainText('waiting');
 await page.locator('[data-ac=stop]').click();await expect(page.locator('[data-ac=recover]')).toBeVisible();await expect(page.locator('[data-ac-checkpoint]')).toContainText('H1');await page.locator('[data-ac=recover]').click();await expect(page.locator('[data-ac-state]')).toContainText('waiting');await expect(page.locator('.ac-observed')).toContainText('Decision H1');
 await page.locator('[data-ac=preview]').click();await expect(page.locator('.ac-prediction')).toContainText('Prediction only');await page.locator('[data-ac=advance]').click();await expect(page.locator('[data-ac-state]')).toContainText('complete');
 await expect(page.locator('[data-ac=extend]')).toBeVisible();await page.locator('[data-ac=more-hours]').fill('1');await page.locator('[data-ac=extend]').click();await expect(page.locator('.ac-observed')).toContainText('Decision H2');await expect(page.locator('[data-ac-state]')).toContainText('waiting');
 await page.locator('[data-ac=preview]').click();await expect(page.locator('.ac-prediction')).toContainText('Prediction only');await page.locator('[data-ac=advance]').click();await expect(page.locator('[data-ac-state]')).toContainText('complete');
 await page.locator('[data-ac=replay]').click();await expect(page.locator('[data-ac-replay]')).toContainText('complete',{timeout:30000});await expect(page.locator('[data-ac-replay]')).toContainText('0 physical differences');await expect(page.locator('[data-ac-state]')).toContainText('3 / 3 h');
