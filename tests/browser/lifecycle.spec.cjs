...
test('recorded lifecycle links preserve the selected component and interval',async({page})=>{
 test.skip(!process.env.DISPATCH_LIFECYCLE_TRACE,'Requires a captured lifecycle run.');await page.goto('/');await page.locator('.m-plant').waitFor();const scrubber=page.locator('[data-m=scrubber]');await page.getByRole('button',{name:'Playback details',exact:true}).click();await scrubber.evaluate(n=>{n.value='12';n.dispatchEvent(new Event('input',{bubbles:true}));});await page.getByRole('button',{name:'Playback details',exact:true}).click();await page.locator('[data-component=electrolyser]').click();
 const link=page.getByRole('button',{name:'Condition & maintenance',exact:true});await expect(link).toBeVisible();await link.click();await expect(page.locator('.d-reference')).toContainText('Original lifecycle decision',{timeout:20000});await expect(page.locator('.d-context')).toContainText('Greedy · interval 11');await expect(page.locator('[data-d=context]')).toHaveValue('This run');await expect(page.locator('.d-reference')).toContainText('retrospective_condition');await page.keyboard.press('Escape');await expect(page.locator('.d-workspace')).toBeHidden();await expect(scrubber).toHaveValue('12');await expect(link).toBeFocused();await page.getByRole('button',{name:'Commissioning record',exact:true}).click();await expect(page.locator('.d-reference')).toContainText('recorded_interval');await page.keyboard.press('Escape');await expect(scrubber).toHaveValue('12');
});
test('simple lifecycle examples meet input and render budgets during batch work',async({page})=>{
 test.skip(!process.env.DISPATCH_LIFECYCLE_PERFORMANCE,'Dedicated reference-machine workload.');test.setTimeout(120000);
 await page.goto('/');await page.locator('.m-plant').waitFor();await page.locator('[data-do=menu]').click();await page.locator('[data-do=analysis]').click();await page.getByRole('tab',{name:'Experiments',exact:true}).click();await page.getByRole('button',{name:'Run / resume suite',exact:true}).click();await page.waitForTimeout(700);await page.getByRole('button',{name:'← Simulation',exact:true}).click();await page.locator('[data-do=menu]').click();await page.locator('.m-utility [data-model-topic=battery]').click();await expect(page.locator('.d-result-status')).toContainText('Learning example');
 const results={};const percentile=a=>[...a].sort((x,y)=>x-y)[Math.floor(a.length*.95)];
 for(const key of ['deployment','condition','hardware']){
  await topic(page,key);await expect(page.locator('.d-result-status')).toContainText('Learning example');await page.evaluate(()=>performance.clearMeasures('model-render'));const samples=[];
  for(let i=0;i<24;i++)samples.push(await page.evaluate(({key,i})=>new Promise(resolve=>{
   const status=document.querySelector('.d-result-status'),started=performance.now();const observer=new MutationObserver(()=>{if(status.textContent.startsWith('Learning example')){observer.disconnect();resolve(performance.now()-started);}});observer.observe(status,{subtree:true,childList:true,characterData:true});
   const set=(name,value)=>{const n=document.querySelector(`[data-input=${name}]`);n.value=String(value);n.dispatchEvent(new Event('input',{bubbles:true}));};
   if(key==='deployment'){set('work',1+i%12);set('crew',1+Math.floor(i/12));}else if(key==='condition')set('age',i*1000);else{const n=document.querySelector('[data-input=family]');set('family',n.options[i%14].value);set('support',i<14?'present':'missing');}
  }),{key,i}));
  const render=await page.evaluate(()=>performance.getEntriesByName('model-render').map(x=>x.duration));results[key]={samples_ms:samples,render_samples_ms:render,input_p95_ms:percentile(samples),render_p95_ms:percentile(render)};
 }
 require('node:fs').writeFileSync('build/release-2/lifecycle-performance.json',JSON.stringify({fixture_hours:Number(await page.locator('[data-m=scrubber]').getAttribute('max')),batch_requested:true,results},null,2));
 await page.keyboard.press('Escape');await page.locator('[data-do=analysis]').click();await page.getByRole('tab',{name:'Experiments',exact:true}).click();await page.getByRole('button',{name:'Cancel suite',exact:true}).click();
 for(const r of Object.values(results)){expect(r.input_p95_ms).toBeLessThanOrEqual(200);expect(r.render_p95_ms).toBeLessThanOrEqual(10);}
});
