...
test('lifecycle essay visual review artifacts',async({page})=>{
 await open(page);for(const key of ['deployment','condition','hardware','maintenance']){await topic(page,key);if(key==='maintenance')await page.locator('[data-d=calculate]').click();await expect(page.locator('.d-result-status')).toContainText('Learning example',{timeout:20000});await page.screenshot({path:`build/release-2/${key}-essay.png`});}
});
