...
def build():
    ...
    gates=dict(source_comparison=read(ROOT/'source-comparison.json',{}).get('complete',False) and read(ROOT/'source-comparison.json',{}).get('passed',False) and read(ROOT/'source-comparison.json',{}).get('after_source')==manifest['source']['content_hash'],numerical_cases=len(cases)==116 and all(c['independent']['status']=='passed' for c in cases),preservation=len(preservation)==6 and all(v.get('matched_traces')==v.get('total') and v.get('offline',{} ) and v['offline']['complete_archives_passed'] for v in preservation),site_bundles=all(g['publication'] and g['publication']['restored_reference']['status']=='passed' for g in studies),qualification=qualification.get('passed',False) and qualification.get('source')==manifest['source']['content_hash'],new_offline=read(ROOT/'offline.json',{}).get('offline_passed',False) and read(ROOT/'offline.json',{}).get('source',{}).get('content_hash')==manifest['source']['content_hash'],observability=read(ROOT/'observability.json',{}).get('passed',False) and read(ROOT/'observability.json',{}).get('source',{}).get('content_hash')==manifest['source']['content_hash'])
