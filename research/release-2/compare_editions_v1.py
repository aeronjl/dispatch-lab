...
from methane.siting.production import entries,PeriodRows,state
...
for group,count in (('seasonal',72),('annual',1)):
 ...
 for ac,bc in zip(before['cases'][:count],after['cases'][:count],strict=True):
  ...
  while True:
   ...
   time.sleep(15)
  ar=PeriodRows(store,prior,ac['controller']);br=PeriodRows(store,current,bc['controller']);ah,bh=[],[];different=[]
  for i in range(hours):
   x={k:v for k,v in ar[i].items() if k in fields};y={k:v for k,v in br[i].items() if k in fields};ah.append(digest(x));bh.append(digest(y))
