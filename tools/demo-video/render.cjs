...
async function main(){
    ...
    try{
        ...
        const seconds=preview?[1,6.5,11.5,13.7,18,21.5,26,28.5,30.5,31.5,33.5,35.5,39]:Array.from({length:story.fps*story.duration},(_,i)=>i/story.fps);
        const ledger=[];
        let closed;
        if(!preview){
            encoder=spawn('ffmpeg',['-y','-loglevel','warning','-f','image2pipe','-framerate',String(story.fps),'-vcodec','mjpeg','-i','pipe:0','-an','-vf','scale=in_range=pc:out_range=tv:out_color_matrix=bt709,format=yuv420p','-c:v','libx264','-preset','slow','-crf','17','-g','60','-pix_fmt','yuv420p','-movflags','+faststart','-color_range','tv','-color_primaries','bt709','-color_trc','bt709','-colorspace','bt709',path.join(out,basename+'.mp4')],{stdio:['pipe','ignore',fs.openSync(path.join(out,'encoder.log'),'w')]});
