...
function renderFieldOperations(result, frame, costs, visual) {
    ...
    const periodNote=result.continuous_period?`<p class="m-muted">This view starts at plant H${result.continuous_period.start_hour}. Playback hours are relative to this saved window; mission deadlines retain their plant hours.</p>`:'';
    const live=periodNote+(underway.length?`<p>In progress during H${frame.hour} → H${frame.hour+1}: ${underway.map(a=>esc(a.kind+' · '+a.phase+' · '+a.order)).join('; ')}.</p><p class="m-muted">Animation uses recorded starting work orders. Completed quantities below stop at H${frame.hour}; work outcomes are not yet available.</p>`:'');
    if(!record)return `<h3>Site services</h3>${live}<p>${frame.hour===0&&result.config.field_operations?.enabled?(result.continuous_period?'Step forward to inspect the carried service state in this saved window.':'Step forward to inspect an executed interval. Robots start with the full battery inventories declared in setup.'):'This run has no recorded field operations. Enable the service layer in experiment setup, or choose a field-services preset.'}</p><p>${esc(result.config.faults?.lifecycle||'Archived timed fault behaviour')} · ${esc(result.config.faults?.capacity_cause||'Original fault assumptions')}</p><button data-do="setup">Experiment setup ↗</button>`;
    ...
    if(beliefs){out+='<h4>What remains uncertain</h4><p>'+esc(beliefs.scope)+'</p>';for(const [key,v] of Object.entries({...beliefs.durations,...Object.fromEntries(Object.entries(beliefs.equipment_durations||{}).flatMap(([asset,groups])=>Object.entries(groups).filter(([,v])=>v.independent_jobs).map(([key,v])=>[asset+" / "+key,v])))})){if(v.completed_phases||v.censored_phases)out+=fact(key+' time · estimate / support',n(v.mean_factor,2)+'× / '+n(v.bounds[0],2)+'–'+n(v.bounds[1],2)+'×')+fact('Observed / unfinished phases',n(v.completed_phases,0)+' / '+n(v.censored_phases,0))+fact('Evidence',v.status);}for(const [key,v] of Object.entries(beliefs.reliability))out+=fact(key+' · completed / interrupted',n(v.returned,0)+' / '+n(v.interrupted,0))+fact('Recovery awaiting verification',n(v.pending_verifications,0));}
    ...
    out+=`<nav class="d-inspector-links"><button data-model-topic="recovery">How service recovery is modelled</button><button data-model-topic="recovery" data-model-context="This run">Trace this service decision</button></nav>`;
    return out;
}
