"""Cancellable process-policy comparisons, bound to run/decision/client generation."""
...
def _reply(entry, job_id):
    ...
    return {
        **base,
        "status": "failed",
        "error": "Policy comparison worker failed; no replacement prediction is available.",
    }
...
def handle(request: Request):
    ...
    with _lock:
        ...
        if request.operation == "poll":
            ...
            if entry is None or entry["context"] != context:
                raise HTTPException(
                    410, "Policy comparison belongs to another selection or has expired"
                )
        ...
        if sum(e["worker"].poll() is None for e in _jobs.values()) >= _MAX_WORKERS:
            return dict(
                key=request.key,
                status="busy",
                error="Two policy comparisons are active; retry shortly.",
            )
        directory = tempfile.TemporaryDirectory(prefix="dispatch-control-comparison-")
        root = Path(directory.name)
        try:
            (root / "input.json").write_text(json.dumps(packet, allow_nan=False))
            env = {
                **os.environ,
                "OMP_NUM_THREADS": "1",
            ...
            with (root / "worker.log").open("w") as log:
                worker = subprocess.Popen(
                    [sys.executable, "-m", "methane.control_worker", str(root)],
        ...
        _jobs[identifier] = dict(
            ...
            directory=directory,
            stopped=None,
            # Three policy rollouts plus startup/postsolve allowance. This is
            # wall time for the comparison, distinct from each solver limit.
            deadline=monotonic() + max(30, 3 * packet["seconds"] + 20),
        )
        timer = threading.Timer(max(30, 3 * packet["seconds"] + 20), _expire, (identifier,))
        timer.daemon = True
